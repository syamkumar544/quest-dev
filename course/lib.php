 ...
 * with or without courses included
 */
function print_category_list($category=NULL, $displaylist=NULL, $parentslist=NULL, $depth=-1,
                             $showcourses = true, $all_categories = array(), $current_categories =array())
    {
    global $CFG;
    ...
    if ($categories = get_child_categories($category->id)) {   // Print all the children recursively
        ...
        $last = false;
        foreach ($categories as $cat) {
            $cat_parents = explode('/',$cat->path);
            unset($cat_parents[count($cat_parents)-1]);

            if(in_array($cat->id,$all_categories) || array_key_exists($cat->id,$current_categories) ||
               array_intersect($cat_parents,array_keys($current_categories))) {
            ...
            print_category_list($cat, $displaylist, $parentslist, $depth + 1, $showcourses, $all_categories, $current_categories);
        }
    }
}
