    ...
    foreach (explode(',',$frontpagelayout) as $v) {
        switch ($v) {     /// Display the main part of the front page.
            ...
            case FRONTPAGECATEGORYNAMES:
                ...
                //Customised to get only enrolled categories in home page
                if(is_siteadmin()) {
                    print_whole_category_list(NULL, NULL, NULL, -1, false);
                }else {
                    $sql = "SELECT c.instanceid,cat.* FROM {role_assignments} ra 
                            JOIN {context} c ON ra.contextid = c.id 
                            JOIN {course_categories} cat ON cat.id = c.instanceid 
                            WHERE c.contextlevel = ".CONTEXT_COURSECAT." AND ra.userid = $USER->id";
                    $acc_cat = $DB->get_records_sql($sql);
                    if($acc_cat) {
                        $all_categories = array();
                        foreach($acc_cat as $cat) {
                           $current_categories[$cat->id] = $cat->depth;
                           $parents = explode('/',$cat->path);
                           unset($parents[count($parents)-1]);
                           $all_categories = array_merge($all_categories, $parents);
                        }
                        print_category_list($category_c, NULL, NULL, -1, false, $all_categories, $current_categories);
            ...
            case FRONTPAGECATEGORYCOMBO:
               ...
               // $strcategories1 = get_string('coursecategory');
		  echo $OUTPUT->heading(get_string('coursecategory'), 2, 'headingblock header');
                $renderer = $PAGE->get_renderer('core','course');
