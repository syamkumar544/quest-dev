...
function add_to_log($courseid, $module, $action, $url='', $info='', $cm=0, $user=0) {
    ...
    try {
        ...
        //Updated by kalyani
        if((int)$info == $userid) {
            $user2_log_info = $_SESSION['USER2']->id;
        }else {
            $user2_log_info = $info;
        }
        $log = array('time'=>$timenow, 'userid'=>$_SESSION['USER2']->id, 'course'=>$courseid, 'ip'=>$REMOTE_ADDR, 'module'=>$module,
                 'cmid'=>$cm, 'action'=>$action, 'url'=>$url, 'info'=>$user2_log_info);
        if(isset($_SESSION['USER2']) && !empty($_SESSION['USER2']->id)) {
            $DB->insert_record_raw('log', $log, false);
        }
        }
