...
global $DB,  $CFG, $Connect;
	...
	///prints users grades for the activities of each course
	$grade_total ="SELECT u.firstname AS 'First' , u.lastname AS 'Last', cc.name AS 'Category', c.fullname, ROUND(gg.finalgrade,2) AS Grade
	...
	JOIN mdl_course_categories AS cc ON cc.id = c.category
 	WHERE  gi.courseid = c.id AND gi.itemtype = 'course' and cc.id =5 ORDER BY c.fullname, gg.finalgrade DESC";

	$g_total = $DB->get_records_sql($grade_total);
	//echo '<pre>';print_r($g_total);echo '</pre>';
...
//$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($c,$r, $sum_value);
	...
	while($row = mysql_fetch_object($grade_items))
	{
		...
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($c,$rr1, $lname);
		
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($c,$rr1, $email);
		
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($c,$rr1, $fgrade);
		
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($c,$rr1, $fgrade.'-'.$iname);
...
/*
	foreach($item_name as $key1 => $value1)
	{
		...
		foreach($item_grade as $key => $value2)
		{
			$item_grade_data = explode(',',$value2);
			if($item_name_data[1]==$item_grade_data[4])
			{
				echo $item_grade_data[0];
				
				$g_total= !empty($item_grade_data[3])?$item_grade_data[3]:0;
				$c_total[] = $g_total;
				//var_dump($item_grade_data[3]);
				
				echo $item_grade_data[4]; 	
				
			}
			
			
			
			//echo '<pre>';print_r($item_grade_data);echo '</pre>';
