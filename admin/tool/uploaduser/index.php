...
$STD_FIELDS = array('id', 'firstname', 'lastname', 'username', 'email',
        ...
        'deleted','prefix',     // 1 means delete user
    );
...
$mform2 = new admin_uploaduser_form2(null, array('columns'=>$filecolumns, 'data'=>array('iid'=>$iid, 'previewrows'=>$previewrows, 'category' => $course_category)));
...
if ($formdata = $mform2->is_cancelled()) {
    ...
    $course_category   = isset($formdata->category) ? $formdata->category : 0;
    ...
    while ($line = $cir->next()) {
        ...
        if ($existinguser) {
            ...
            if(!isset($existinguser->prefix) || empty($existinguser->prefix)) {
                $existinguser->prefix = $CFG->locationprefix;
            }
            ...
            if(!isset($user->prefix) || empty($user->prefix)) {
                $user->prefix = $CFG->locationprefix;
            }
        ...
        foreach ($filecolumns as $column) {
            ...
            if (!array_key_exists($shortname, $ccache)) {
                ...
                if(!empty($course_category)) {
                     $course = $DB->get_record_sql("SELECT * from {course} WHERE category = $course_category AND fullname like '%".$shortname."%'");
                }else {
                     $course = $DB->get_record('course', array('shortname'=>$shortname), 'id, shortname');
                }
                if(!$course) {
                    $upt->track('enrolments', get_string('unknowncourse', 'error', s($fullname)), 'error');
                    continue;
                }
                $shortname = $course->shortname;
                $ccache[$shortname] = $course;
                $ccache[$shortname]->groups = null;
                $user->{'course'.$i} = $course->shortname;
            }
...
while ($linenum <= $previewrows and $fields = $cir->next()) {
    ...
    foreach($fields as $key => $field) {
        ...
        if(preg_match('/^course\d+$/', $filecolumns[$key]) && !empty($course_category)) {
             $course = $DB->get_record_sql("SELECT id,fullname from {course} WHERE category = $course_category AND fullname like '%".$field."%'");
             if($course) {
                 $rowcols[$filecolumns[$key]] = '<a href="'.$CFG->wwwroot.'/course/view.php?id='.$course->id.'">'.$course->fullname.'</a>';
             }
        }
    }
