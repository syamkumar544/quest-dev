...
class admin_uploaduser_form1 extends moodleform {
    function definition () {
        ...
        $categories = $DB->get_records_sql("SELECT DISTINCT c.category,cat.name,cat.parent FROM {course} c,{course_categories} cat WHERE cat.id = c.category");
        if($categories) {
           $mform->addElement('advcheckbox', 'enroll', 'Enroll','', array('group' => 0), array(0, 1));
           foreach($categories as $category) {
              ...
              $categories_arr[$category->category] = $parent_name.'/'.$category->name;
           }
           $mform->addElement('select', 'category', 'Category', $categories_arr);
           $mform->disabledIf('category', 'enroll', 'eq', 0);
        }
