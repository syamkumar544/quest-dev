...
//Updated by kalyani
    if($frm->username2<>'')
    $frm->username2 = trim(textlib::strtolower($frm->username2));
    ...
    if($frm->username2 != '') {
        if (is_enabled_auth('none') ) {
            if ($frm->username2 !== clean_param($frm->username2, PARAM_USERNAME)) {
                $errormsg = get_string('username').': '.get_string("invalidusername");
                $errorcode = 2;
                $user = null;
            }
    ...
    if ($user) {
        ...
        if (empty($errormsg)) {
            //Updated by kalyani
            if($frm->username2<>'' and $frm->password2<>'' and $frm->username<>'' and $frm->password<>'') {
                $user2 = authenticate_user_login($frm->username2, $frm->password2);
    ...
    /// Let's get them all set up.
        ...
        if($frm->username2<>'' and $frm->password2<>'' and $frm->username<>'' and $frm->password<>'') {
            ...
            if($user2 -> id) {
                ...
                $user_2 = new stdClass();
                $user_2->id = $user2 -> id;
                if ($user2->firstaccess == 0) {
                    $user_2->firstaccess = $now;
                }
                $user_2->lastlogin = $user2->currentlogin;
                $user_2->currentlogin = $now;
                $user_2->lastaccess = $now;
                $user_2->lastip = getremoteaddr();

                $DB->update_record('user', $user_2);
