...
//Updated by kalyani
        if($_SESSION['USER2']) {
            if($grades = scorm_get_user_grades($scorm, $_SESSION['USER2']->id)) {
                scorm_grade_item_update($scorm, $grades);
                scorm_set_completion($scorm, $_SESSION['USER2']->id, COMPLETION_COMPLETE, $grades);
            }
        }
//---------------------------------------
...
//Updated by kalyani
        if($_SESSION['USER2']->id) {
            $user2_grade = new stdClass();
            $user2_grade->userid   = $_SESSION['USER2']->id;
            $user2_grade->rawgrade = null;
            scorm_grade_item_update($scorm, $user2_grade);
            scorm_set_completion($scorm, $_SESSION['USER2']->id, COMPLETION_INCOMPLETE);
        }
        //--------------------------
    } else {
        scorm_grade_item_update($scorm);
